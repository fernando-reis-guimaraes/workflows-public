...
1. Configure reusable workflow policy at organization level:

- Organization Settings -> Actions -> General
- Allow actions and reusable workflows from `ultra-mega-apps/workflows-public`

2. Configure access policy in the shared workflow repository:

- Repository Settings -> Actions -> General (in `workflows-public`)
- Allow access from repositories in your organization

3. Define organization-level variables for caller repositories:

- `FRGINBOX_CAPROVER_URL`
- `STORAGE_CAPROVER_URL`

4. Define organization-level secrets for caller repositories:

- `FRGINBOX_CAPROVER_PASSWORD`
- `STORAGE_CAPROVER_PASSWORD`

5. In caller repositories, reference this reusable workflow with a stable ref (`@main` or a tag) and use `secrets: inherit`.
6. Run one deployment with `server=frginbox` and one with `server=storage` to validate both routes.

### Configure With gh CLI

```bash
# Required token scopes for these operations: repo, read:org, admin:org
gh auth login

ORG="ultra-mega-apps"
SHARED_REPO="workflows-public"

# 1) Organization-level Actions policy for reusable workflows
# Adjust values if you prefer a stricter policy.
gh api -X PUT "/orgs/${ORG}/actions/permissions" \
  -f enabled_repositories="all" \
  -f allowed_actions="all"

# 2) Shared repo access policy (mainly relevant when the shared repo is private)
